...
# ----------------------------- Config -----------------------------
MODEL_PATH_DEFAULT = "lstm_best.h5"  # trained LSTM
...
QD_MAX_DOSE       = 40.0   # mg (initial ceiling; will auto-extend if needed)
QD_STEP           = 0.5    # mg
...
def load_trained_model(path: str):
    ...
    try:
        # Compatibility with legacy 'mse' in H5
...
def load_or_build_scalers(time_scaler_path: str, y_scaler_path: str, data_csv: str):
    if os.path.exists(time_scaler_path) and os.path.exists(y_scaler_path):
...
def mc_success_curve_qd(rng, model, y_scaler, time_scaler, dose_grid, N_base, p_comed, R):
    succ_mat = np.zeros((R, len(dose_grid)))
    dose90   = np.full(R, np.nan)
    for _ in tqdm(range(R), desc="QD MC"):
        ...
        if i90.size:
            dose90[_] = float(dose_grid[i90[0]])
    return succ_mat, dose90

def mc_success_curve_qw(rng, model, y_scaler, time_scaler, dose_grid, N_base, p_comed, R):
    succ_mat = np.zeros((R, len(dose_grid)))
    dose90   = np.full(R, np.nan)
    for _ in tqdm(range(R), desc="QW MC"):
        BW = rng.uniform(BW_RANGE[0], BW_RANGE[1], size=N_base*K_EXPAND)
        COM = rng.binomial(1, p_comed, size=N_base*K_EXPAND)
        s = []
        for d in dose_grid:
...
def summarize_curve(succ_mat, dose_grid, dose90_samples):
    ...
    # minimal dose on the mean curve
...
def run_qd_until_cross(rng, model, y_scaler, time_scaler, N_base,
                       ...
                       p_comed=0.5, R=R_MC_DEFAULT):
    grid = np.arange(start, initial_max + step, step)
    succ_qd, d90_qd = mc_success_curve_qd(rng, model, y_scaler, time_scaler, grid, N_base, p_comed, R=R)
...
def main():
    ...
    parser.add_argument("--data",  type=str, default=DATA_CSV_DEFAULT, help="Path to EstData.csv")
    parser.add_argument("--model", type=str, default=MODEL_PATH_DEFAULT, help="Path to trained LSTM model (.h5 or SavedModel)")
    ...
    parser.add_argument("--mc",   type=int, default=R_MC_DEFAULT, help="MC replicates")
    ...
    # Scalers (re)build/load under outdir
    ...
    qd_grid, succ_qd, d90_qd = run_qd_until_cross(
        ...
        initial_max=args.qd_initial_max, extend_by=20.0,
        max_extends=10, hard_cap=240.0,
        p_comed=p_comed, R=args.mc
    )
