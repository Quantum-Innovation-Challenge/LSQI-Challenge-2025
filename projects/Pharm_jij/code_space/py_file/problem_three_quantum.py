...
# -*- coding: utf-8 -*-
"""
Effect of changing body weight distribution to 70–140 kg on optimal QD/QW doses (QLSTM)
---------------------------------------------------------------------------------------
- Quantum LSTM (PennyLane + PyTorch) surrogate for PD(t)
...
QD_MAX_DOSE        = 40.0   # mg (initial ceiling; auto-extends if needed)
QD_STEP            = 0.5    # mg
...
def mc_success_curve_qd(rng, model, device, y_scaler, time_scaler, dose_grid, N_base, p_comed, R):
...
def mc_success_curve_qw(rng, model, device, y_scaler, time_scaler, dose_grid, N_base, p_comed, R):
    succ_mat = np.zeros((R, len(dose_grid)))
    dose90   = np.full(R, np.nan)
...
def run_qd_until_cross(rng, model, device, y_scaler, time_scaler, N_base,
    ...
    succ_qd, d90_qd = mc_success_curve_qd(rng, model, device, y_scaler, time_scaler, grid, N_base, p_comed, R=R)
...
def main():
    ...
    parser.add_argument("--qubits", type=int, default=N_QUBITS_DEFAULT, help="QLSTM qubits")
    parser.add_argument("--qlayers", type=int, default=N_QLAYERS_DEFAULT, help="QLSTM layers")
